...
import { X, CornerDownLeft, MessageSquare, Sparkles } from "lucide-react";
...
const GLOBAL_QUESTIONS = [
  {
    ...
    a: "Every tool in this lab calls a local Next.js API route handler (like /api/normalize). The Next.js server then forwards the request to our backend Hono REST API service running on port 3001, injecting the required authentication keys in server-to-server calls.",
  },
  {
    q: "How is the infrastructure secured?",
    a: "To protect our backend and database from unauthorized public access, we generate and seed a secure API key (fidel_lab_key_2026) directly into the PostgreSQL database on startup. Next.js routes serve as a proxy, so your browser never leaks the Hono URL or the API keys in your Network tab!",
  },
];
...
const PAGE_CONTEXTS: Record<string, {
  ...
  "/": {
    ...
    greeting: "ሰላም! Welcome to the Fidel NLP Lab overview dashboard. How can I help you get started?",
  ...
  "/search": {
    ...
    answer: "We index documents as stem maps, create an inverted index, and score them using BM25 relevance scores. In Search Showdown, you select cards with the best match!"
  }
};
...
export default function FidelCompanion() {
  ...
  // Whenever path changes, update the initial greeting message dynamically
  useEffect(() => {
    setMessages([
      {
        sender: "fidel",
        text: currentCtx.greeting,
      },
    ]);
  }, [pathname, currentCtx.greeting]);
  ...
  const handleAskQuestion = (qText: string, aText: string) => {
    ...
    const interval = setInterval(() => {
      ...
      if (charIndex >= aText.length) {
        ...
        setMessages((prev) => [...prev, { sender: "fidel", text: aText }]);
        setTypingText("");
        setIsTyping(false);
      }
    }, 12);
  ...
  const activeQuestions = [
    { q: currentCtx.question, a: currentCtx.answer, label: "Page-Specific Context" },
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[999] font-sans">
      ...
      {!isOpen && (
        ...
        >
          <span className="font-loga text-xl font-bold select-none group-hover:rotate-12 transition-transform">ፊ</span>
          ...
          <div className="p-4 border-b border-zinc-250/60 dark:border-zinc-900 bg-zinc-50 dark:bg-zinc-900/40 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-lg bg-blue-500/10 dark:bg-amber-500/10 border border-blue-500/20 dark:border-amber-500/20 flex items-center justify-center">
                <span className="font-loga text-base font-bold text-blue-600 dark:text-amber-500 select-none">ፊ</span>
              </div>
              <div>
          ...
          <div className="p-4 border-t border-zinc-250/60 dark:border-zinc-900 bg-zinc-50 dark:bg-zinc-900/20 space-y-2">
            ...
            <div className="grid grid-cols-1 gap-1.5 max-h-[140px] overflow-y-auto p-0.5">
              {activeQuestions.map((item) => {
                const alreadyAsked = messages.some((m) => m.text === item.q);
                return (
                  <button
                    key={item.q}
