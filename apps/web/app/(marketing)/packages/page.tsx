...
// -> 'ልጅ'`,
    },
    {
      ...
      description:
        "High-performance native core pre-processing module. Houses the Rust implementation of the Amharic normalizer compiled to WebAssembly, featuring zero-dependency base64 inline loading.",
      features: [
      ...
      <div className="flex flex-col lg:flex-row gap-12 lg:gap-16">
        ...
        <div className="lg:w-[35%] lg:sticky lg:top-24 h-fit space-y-4">
          ...
          <p className="text-xs sm:text-sm font-semibold text-slate-650 dark:text-zinc-400 leading-relaxed max-w-sm">
            Fidel Tools is built as a series of modular packages. Install what you need locally,
            validate custom configurations, or query managed cloud telemetry.
          </p>
        </div>
        ...
        <div className="lg:w-[65%] space-y-12">
          {packages.map((pkg, i) => (
            <div
              key={pkg.name}
              className="border border-slate-200 dark:border-zinc-900 bg-white dark:bg-[#070709] rounded-lg overflow-hidden shadow-sm dark:shadow-xl flex flex-col font-sans p-6 space-y-6 transition-colors duration-300"
            >
              {/* Package Header */}
              ...
              <div className="space-y-2.5">
                ...
                <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2">
                  {pkg.features.map((feat, idx) => (
                    <li
                      key={idx}
                      className="flex items-start gap-2 text-xs font-semibold text-slate-705 dark:text-zinc-300"
                    >
