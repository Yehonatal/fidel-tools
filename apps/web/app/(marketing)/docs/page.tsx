...
export default function DocsPage() {
  ...
  const [selectedEndpointId, setSelectedEndpointId] = useState<string>("overview");
  ...
  const selectedEndpoint = useMemo(() => {
    if (selectedEndpointId === "overview") return null;
    return endpoints.find((e) => e.id === selectedEndpointId) || endpoints[0];
  }, [endpoints, selectedEndpointId]) as Endpoint;
...
print(response.json())`;
  ...
  const runTestRequest = async () => {
    if (!isValidJson || isLoading || !selectedEndpoint) return;
    setIsLoading(true);
    setResponseBody(null);
    setResponseStatus(null);
    setExecutionTime(null);
  ...
  return (
    <div className="flex-grow w-full max-w-7xl mx-auto px-4 md:px-6 py-6 md:py-10 grid grid-cols-1 lg:grid-cols-4 gap-8 relative z-10">
      ...
      <aside className="lg:col-span-1 space-y-6">
        ...
        <div className="space-y-6 max-h-[calc(100vh-160px)] overflow-y-auto pr-2 custom-scrollbar">
          ...
          <div className="space-y-2">
            <h3 className="text-[10px] font-bold font-mono tracking-widest text-zinc-555 dark:text-zinc-500 uppercase">
              Welcome
            </h3>
            <div className="space-y-1">
              <button
                onClick={() => {
                  setSelectedEndpointId("overview");
                  setActiveTab("documentation");
                }}
                className={`w-full flex items-center justify-between p-2 rounded-md transition-all text-left text-xs font-bold ${
                  selectedEndpointId === "overview"
      ...
      <main className="lg:col-span-3 grid grid-cols-1 xl:grid-cols-5 gap-8">
        ...
        {selectedEndpointId === "overview" ? (
          /* General API Overview Page */
          <section className="xl:col-span-5 space-y-6 animate-fade-in">
            {/* Overview Header */}
            <div className="space-y-3 pb-5 border-b border-slate-200/50 dark:border-zinc-900">
              <h1 className="text-3xl font-light text-zinc-900 dark:text-white tracking-tight flex items-center gap-2">
                <span className="font-loga text-3xl font-light mr-4">ፊደል</span>
                <span className="font-sans font-bold text-2xl text-zinc-900 dark:text-white">
                  (Fidel) Tools API Reference
                </span>
              </h1>
              <p className="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed max-w-3xl font-medium">
                <span className="font-loga font-light text-[15px] mr-3">ፊደል</span>
                <span className="font-sans">
                  (Fidel) Tools is a developer-first suite of high-performance natural language processing APIs built specifically for Ethiopic languages. This reference provides interactive documentation for our production-grade NLP preprocessing endpoints.
                </span>
              </p>
            </div>

            {/* Quick Start Details Grid */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
              
              {/* Base URLs */}
              <div className="p-5 rounded-xl border border-zinc-200 dark:border-zinc-900 bg-white dark:bg-[#070709] space-y-3">
                <div className="inline-flex p-2 rounded bg-blue-500/10 text-blue-500 border border-blue-500/15">
                ...
                <div className="space-y-3 pt-1">
                  {/* Production Endpoint (Blurred / Construction Overlay) */}
                  <div className="relative group border border-slate-200/60 dark:border-zinc-900/60 rounded-lg overflow-hidden bg-slate-50/50 dark:bg-zinc-950/40 p-2.5">
                    <span className="text-[9px] font-bold font-mono text-zinc-400 dark:text-zinc-500 uppercase tracking-wide">
                      Production Base URL
                    </span>
                    <div className="mt-1 font-mono text-xs font-bold text-zinc-850 dark:text-zinc-200 select-none blur-sm group-hover:blur-[1px] transition-all duration-200">
                      https://api.fidel.tools
                    </div>
                    {/* Hover Overlay */}
                    <div className="absolute inset-0 bg-black/60 dark:bg-black/80 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200 cursor-not-allowed">
                      <span className="text-[9px] font-bold text-amber-500 uppercase tracking-wider font-mono px-2 text-center">
                  ...
                  <div className="border border-slate-200/60 dark:border-zinc-900/60 rounded-lg bg-slate-50/50 dark:bg-zinc-950/40 p-2.5">
                    <span className="text-[9px] font-bold font-mono text-zinc-400 dark:text-zinc-500 uppercase tracking-wide">
                      Local Development Base URL
                    </span>
                    <pre className="mt-1 text-xs font-mono font-bold text-zinc-800 dark:text-zinc-200 overflow-x-auto select-all bg-transparent border-0 p-0 rounded-none shadow-none">
                      http://localhost:3001
                    </pre>
                  </div>
                </div>
              ...
              {/* Authentication */}
              <div className="p-5 rounded-xl border border-zinc-200 dark:border-zinc-900 bg-white dark:bg-[#070709] space-y-3">
                <div className="inline-flex p-2 rounded bg-amber-500/10 text-amber-500 border border-amber-500/15">
                ...
                <div className="space-y-2 pt-1">
                  <div>
                    ...
                    <pre className="mt-1 p-2 bg-slate-50 dark:bg-zinc-950 text-xs font-mono font-bold text-zinc-800 dark:text-zinc-200 rounded border border-slate-100 dark:border-zinc-900/60 overflow-x-auto select-all">
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-50 dark:bg-zinc-950 border-b border-slate-200 dark:border-zinc-900">
                      <th className="p-3 text-[10px] font-bold uppercase font-mono text-zinc-500 dark:text-zinc-400 w-24">
                        Code
                      </th>
                      <th className="p-3 text-[10px] font-bold uppercase font-mono text-zinc-500 dark:text-zinc-400 w-44">
                        Status
                      </th>
                      <th className="p-3 text-[10px] font-bold uppercase font-mono text-zinc-500 dark:text-zinc-400">
                        Description
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr className="border-b border-slate-100 dark:border-zinc-900/50 hover:bg-slate-50/50 dark:hover:bg-zinc-950/20">
                      <td className="p-3 font-mono text-xs font-bold text-emerald-600 dark:text-emerald-400">
                        200
                      </td>
                      <td className="p-3 font-mono text-xs font-bold text-zinc-900 dark:text-zinc-300">
                        OK
                      </td>
                      <td className="p-3 text-xs text-zinc-500 dark:text-zinc-400 font-semibold leading-relaxed">
                        Request completed successfully and returned requested payload data.
                      </td>
                    </tr>
                    <tr className="border-b border-slate-100 dark:border-zinc-900/50 hover:bg-slate-50/50 dark:hover:bg-zinc-950/20">
                      <td className="p-3 font-mono text-xs font-bold text-red-500">
                        400
                      </td>
                      <td className="p-3 font-mono text-xs font-bold text-zinc-900 dark:text-zinc-300">
                        Bad Request
                      ...
                      <td className="p-3 text-xs text-zinc-500 dark:text-zinc-400 font-semibold leading-relaxed">
                        Validation failed or request JSON body does not match endpoint schemas.
                      </td>
                    </tr>
                    <tr className="border-b border-slate-100 dark:border-zinc-900/50 hover:bg-slate-50/50 dark:hover:bg-zinc-950/20">
                      <td className="p-3 font-mono text-xs font-bold text-red-500">
                      ...
                      <td className="p-3 text-xs text-zinc-500 dark:text-zinc-400 font-semibold leading-relaxed">
                        Missing, invalid, or inactive API key passed in headers.
                      </td>
                    </tr>
                    <tr className="border-b border-slate-100 dark:border-zinc-900/50 hover:bg-slate-50/50 dark:hover:bg-zinc-950/20">
                      <td className="p-3 font-mono text-xs font-bold text-red-500">
                      ...
                      <td className="p-3 text-xs text-zinc-500 dark:text-zinc-400 font-semibold leading-relaxed">
                        Rate limit exceeded for developer key or IP address (public rate limiting).
                      </td>
                    </tr>
                    <tr className="border-b border-slate-100 dark:border-zinc-900/50 hover:bg-slate-50/50 dark:hover:bg-zinc-950/20">
                      <td className="p-3 font-mono text-xs font-bold text-red-500">
                      ...
                      <td className="p-3 text-xs text-zinc-500 dark:text-zinc-400 font-semibold leading-relaxed">
                        An unhandled exception or database connection error occurred on the API server.
                      </td>
                    </tr>
                  </tbody>
                </table>
            ...
            <section className="xl:col-span-3 space-y-6">
              ...
              {activeTab === "documentation" ? (
                            ...
                            {Object.entries(selectedEndpoint.requestBody).map(([name, schema]) => (
                              ...
                              >
                                <td className="p-3">
                                  ...
                                  <p className="text-[11px] text-zinc-500 dark:text-zinc-400 mt-1 leading-relaxed font-semibold">
                                    {schema.description}
                                  </p>
                                </td>
                ...
                <div className="space-y-6">
                  ...
                  {selectedEndpoint.auth && (
                    <div className="space-y-2">
                      <label className="text-[10px] font-bold font-mono tracking-wider text-zinc-500 dark:text-zinc-450 uppercase flex items-center gap-1">
                  ...
                  {Object.keys(selectedEndpoint.requestBody).length > 0 && (
                    <div className="space-y-2">
                      <div className="flex justify-between items-center">
                        <label className="text-[10px] font-bold font-mono tracking-wider text-zinc-555 dark:text-zinc-455 uppercase">
                  ...
                  {(responseStatus !== null || responseBody !== null) && (
                    <div className="space-y-3 pt-5 border-t border-slate-200 dark:border-zinc-900">
                      <div className="flex items-center justify-between">
                        <h3 className="text-xs font-bold font-mono tracking-widest text-zinc-500 dark:text-zinc-450 uppercase">
                          Response Output
                        </h3>
                        <div className="flex items-center gap-3">
                          {executionTime !== null && (
